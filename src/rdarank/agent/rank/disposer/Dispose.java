package rdarank.agent.rank.disposer;
 ...
 *  エージェントはDISPOSEメッセージを受信すると，自ら削除処理を行う．
...
public class Dispose implements AgentExecutor, Serializable {
	 ...
	 *
	 */
	private static final long serialVersionUID = 4920252877911662281L;

	@Override
	/**
	 * 全エージェントからのDISPOSEメッセージの返答を取得し，そのまま返す．
	 * resultsは，キーがエージェントキー，値がエージェントからの返答文字列
	 * であるHashMapのコレクションである．HashMapはエージェント実行環境
	 * ごとに生成されたものである．
	 */
	...
	public Object execute() {
            try {
                ...
                Message msg = factory.getMessage("disposeRankAgent");
